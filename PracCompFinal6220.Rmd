...
ggplot(data=gucplant.Aseries, aes(x=Shoot.Length, y=Root.Length)) +
  geom_point(color="Red", linewidth=2, alpha=0.9) +
  theme_ipsum() +
  ggtitle("Tank Root Growth for Session 1")
```
...
```{r}
gucplant.Aseries.ANOVA <- aov(gucplant.Aseries$Shoot.Length ~ gucplant.Aseries$Species)
summary(gucplant.Aseries.ANOVA)
```

```{r}
TukeyHSD(gucplant.Aseries.ANOVA)
```
...
```{r}
ggplot(data = gucplant.Aseries, mapping = aes())
```
