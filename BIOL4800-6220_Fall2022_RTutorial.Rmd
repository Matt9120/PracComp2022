...
```
To transfer an expoential graph to use a regression line, use log10()
```{r}
...
```
```{r}
spider_data <-read.csv(file="https://wsc.nmbe.ch/resources/species_export_20221103.csv", header=TRUE)
spider_data
```
```{r}
library(ggridges)
...
#### Bivariate and Multivariate Stats
